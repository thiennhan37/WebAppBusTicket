...
import {ArrowLeft, Calendar, Clock, MapPin, Star, AlertCircle, ShieldAlert, Award, Heart} from "lucide-react";
...
export default function OrderDetailPage() {
    ...
    // Check if current order is in favorites on load
    useEffect(() => {
        if (!orderDetail) return;
        const favorites = JSON.parse(localStorage.getItem("favoriteTrips") || "[]");
        const exists = favorites.some((f) => f.bookingOrderId === orderDetail.bookingOrderId);
        setIsFavorite(exists);
    }, [orderDetail]);

    const toggleFavorite = () => {
        if (!orderDetail) return;
        let favorites = JSON.parse(localStorage.getItem("favoriteTrips") || "[]");
        const exists = favorites.some((f) => f.bookingOrderId === orderDetail.bookingOrderId);

        if (exists) {
            favorites = favorites.filter((f) => f.bookingOrderId !== orderDetail.bookingOrderId);
            setIsFavorite(false);
            toast.success("Đã xóa khỏi danh sách yêu thích");
        } else {
            favorites.push({
                bookingOrderId: orderDetail.bookingOrderId,
                busCompanyName: orderDetail.busCompanyName,
                busType: orderDetail.busType,
                pickupProvince: orderDetail.pickupProvince,
                pickupStop: orderDetail.pickupStop,
                dropoffProvince: orderDetail.dropoffProvince,
                dropoffStop: orderDetail.dropoffStop,
                departureTime: orderDetail.departureTime,
                totalAmount: orderDetail.totalAmount,
                seatCount: orderDetail.seatCount,
                seatCodes: orderDetail.seatCodes,
    ...
    return (
        <div className="order-detail-page">
            <div className="container">
                <button className="brutal-btn" onClick={() => navigate("/khachhang/don-hang")}
                        ...
                        <div className="order-main-info">
                            <BrutalCard className="detail-card" noHover>
                                <div className="detail-card-header">
                                    <div>
                                        <h3 className="detail-card-title text-mono">Chi tiết đơn hàng
                                            #{orderDetail?.bookingOrderId}</h3>
                                        <div
                                            className="detail-company text-mono">{orderDetail?.busCompanyName} ({orderDetail?.busType})
                                        </div>
                                    </div>
                                    <div className="detail-card-header-actions">
                                        ...
                                        >
                                            <Heart
                                                ...
                                                color={isFavorite ? "#e74c3c" : "var(--color-gray-400)"}
                                            />
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div className="detail-time-grid">
                                    <div className="time-grid-item">
                                        ...
                                        <div>
                                            ...
                                            <div
                                                className="grid-val text-mono">{formatDate(orderDetail?.departureTime)}</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="time-grid-item">
                                        ...
                                        <div>
                                            ...
                                            <div
                                                className="grid-val text-mono">{formatTime(orderDetail?.departureTime)}</div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="order-sidebar-action">
                            ...
                            {((orderDetail?.orderStatus || "").toUpperCase() === "PENDING" ||
                                ...
                                <BrutalCard className="payment-widget-card" noHover>
                                    ...
                                    <p className="widget-desc">Đơn hàng của bạn đang chờ thanh toán. Hãy chọn phương
                                        thức bên dưới.</p>

                                    <div className="payment-options-widget">
                                        <label
                                            className={`payment-option-row ${paymentMethod === "vnpay" ? "payment-option-row--selected" : ""}`}>
                                            <input
                                                type="radio"
                                        ...
                                        <label
                                            className={`payment-option-row ${paymentMethod === "momo" ? "payment-option-row--selected" : ""}`}>
                                            <input
                                                type="radio"
                                                name="paymentMethodWidget"
                                                value="momo"
                                    ...
                                    {ratingDetail && (
                                        <div className="rating-result">
                                            <div className="average-rating-indicator">
                                                <Star size={20} fill="var(--color-yellow)"
                                                      className="star-icon--active"/>
                                                <span
                                                    className="avg-val">{ratingDetail.averageStars?.toFixed(1) || "5.0"}</span>
                                                <span>/ 5.0</span>
                                            </div>
                                            ...
                                            {ratingDetail.description && (
                                                <div className="rating-comment brutal-card" style={{
                                                    marginTop: "1rem",
                            ...
                            {(orderDetail?.orderStatus || "").toUpperCase() === "CANCELLED" && (
                                <BrutalCard className="cancelled-widget-card" noHover>
                                    ...
                                    <p className="widget-desc" style={{fontSize: "var(--font-size-xs)"}}>
                                        Đơn hàng đã được giải phóng ghế do hết thời gian giữ chỗ hoặc do bạn chủ động
                                        hủy. Vui lòng đặt chuyến khác!
                                    </p>
                                    <Link to="/khachhang" className="brutal-btn brutal-btn--primary brutal-btn--full"
