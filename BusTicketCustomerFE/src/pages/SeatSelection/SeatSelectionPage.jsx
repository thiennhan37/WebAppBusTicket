...
export default function SeatSelectionPage() {
  ...
  useEffect(() => {
    const fetchDiagram = async () => {
      ...
      try {
        ...
        // api response might wrap result in ApiResponse structure
  ...
  const handleContinue = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await holdSeats(tripId, { tripSeatIdList: selectedSeatIds });
      const orderId = res.data.result || res.data;
      toast.success("Giữ ghế thành công!");
      navigate(`/khachhang/dat-ve/${tripId}?orderId=${orderId}`, {
        state: {
          selectedSeatCodes,
      ...
      const errMsg = err.response?.data?.message || "Lỗi giữ ghế. Có thể ghế đã được chọn bởi người khác!";
      toast.error(errMsg);
    } finally {
      setSubmitting(false);
    }
            ...
            <div className="seat-selection-summary">
              <BrutalCard className="summary-card" noHover>
                ...
                <div className="summary-card__note">
                  <HelpCircle size={14} />
                  <span>
                    Mỗi tài khoản được chọn tối đa 5 ghế. Ghế sẽ được giữ trong vòng 10 phút để thanh toán.
                  </span>
                </div>
              </BrutalCard>
            </div>
